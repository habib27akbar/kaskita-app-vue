  ...
  <q-page class="q-pa-md bg-grey-2">
    <q-card class="q-pa-md">
      ...
      <q-input
        ...
        class="q-mb-md"
      />
      <q-list bordered separator v-if="jurnalUmumList.length > 0">
        <q-item v-for="item in jurnalUmumList" :key="item.id" clickable>
          <q-item-section>
...
const $q = useQuasar()
const LOCAL_KEY = 'jurnal_umum_data'
const jurnalUmumList = ref([])
...
// =======================
// Offline-first helpers (seperti pembelian)
// =======================
...
function dedupeByIdPreferLocal(arr) {
  ...
  for (const item of arr) {
    ...
    // sama2 synced → pilih terbaru
    const t = (o) => new Date(o.updated_at || o.created_at || o.tanggal_debet || 0).getTime()
    if (t(item) > t(prev)) map.set(key, item)
  }
  return Array.from(map.values())
}
function normalizeForView(arr) {
  // kunci tampilan pakai local_id kalau ada
  const m = new Map()
  for (const it of arr) {
    ...
    const ta = new Date(it.created_at || it.tanggal_debet || 0).getTime()
    const tb = new Date(prev.created_at || prev.tanggal_debet || 0).getTime()
...
async function fullSyncAllPages() {
  ...
  const all = []
  while (true) {
    const resp = await api.get(`${API_URL}/jurnal_umum`, {
  ...
  jurnalUmumList.value = normalizeForView(after.filter(viewFilter))
  pagination.total = jurnalUmumList.value.length
}
function toInt(v) {
...
async function fetchData() {
  ...
  if (isOffline()) {
    ...
    jurnalUmumList.value = normalizeForView(cached)
    pagination.total = jurnalUmumList.value.length
    loading.value = false
    return
  }
  ...
  try {
    const response = await api.get(`${API_URL}/jurnal_umum`, {
      params: {
        email: currentEmail,
    ...
    jurnalUmumList.value = normalizeForView(merged.filter(viewFilter))
    pagination.total = jurnalUmumList.value.length

    // coba sinkron yg pending (kalau ada)
    ...
    console.error(e)
    const cached = getCache()
    jurnalUmumList.value = normalizeForView(cached.filter(viewFilter))
    pagination.total = jurnalUmumList.value.length
    $q.notify({
      type: cached.length ? 'warning' : 'negative',
      message: cached.length
        ? 'Mode offline: menampilkan data dari cache (jurnal umum)'
    ...
    try {
      if (rec.__op === 'delete') {
        if (!String(rec.id).startsWith('local_'))
          await api.delete(`${API_URL}/jurnal_umum/${rec.id}`)
        cache = cache.filter((x) => x.id !== rec.id)
        continue
      }
      if (rec.__op === 'update') {
        if (!String(rec.id).startsWith('local_')) {
          await api.put(`${API_URL}/jurnal_umum/${rec.id}`, buildPayload(rec))
          ...
          const res = await api.post(`${API_URL}/jurnal_umum`, buildPayload(rec))
      ...
      console.error('Sync jurnal umum error:', err)
      $q.notify({
        type: 'warning',
        message: 'Sebagian data offline gagal tersinkron (jurnal umum).',
      })
    }
  }
...
async function saveData() {
  ...
  const saveOfflineLocal = () => {
    ...
    setCache(cache)
    jurnalUmumList.value = normalizeForView(getCache())
    pagination.total = jurnalUmumList.value.length
  }
  ...
  try {
    if (isOffline()) {
      ...
      if (isEdit) {
        const res = await api.put(`${API_URL}/jurnal_umum/${form.id}`, payload)
        ...
        const res = await api.post(`${API_URL}/jurnal_umum`, payload)
        ...
        setCache(updated)
        jurnalUmumList.value = normalizeForView(updated.filter(viewFilter))
        pagination.total = jurnalUmumList.value.length
        pagination.page = 1
      }
    }
...
function deleteData(id) {
  const item = jurnalUmumList.value.find((p) => p.id === id)
  //console.log('ITEM YANG AKAN DIHAPUS:', item)
  const nilaiHutang = item ? ` (Jurnal Umum : Rp ${Number(item.debet).toLocaleString()})` : ''
  $q.dialog({
    title: 'Konfirmasi',
    ...
    try {
      if (isOffline()) {
        ...
        setCache(cache)
        jurnalUmumList.value = sortNewestFirst(getCache())
        pagination.total = jurnalUmumList.value.length
        $q.notify({ type: 'warning', message: 'Data dihapus (offline)' })
      } else {
        try {
          await api.delete(`${API_URL}/jurnal_umum/${id}`)
          const cache = getCache().filter((x) => String(x.id) !== String(id))
          setCache(cache)
          jurnalUmumList.value = normalizeForView(cache.filter(viewFilter))
          pagination.total = jurnalUmumList.value.length
