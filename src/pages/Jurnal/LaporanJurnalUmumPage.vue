<template>
  <q-page padding>
    <div class="q-gutter-md">
      <q-card>
        <q-card-section>
          <div class="text-h6">Laporan Jurnal Umum</div>
        </q-card-section>
...
async function filterData() {
  loading.value = true
  try {
    const res = await api.get(`${API_URL}/laporan/jurnal_umum`, {
      params: {
...
const exportToExcel = () => {
  ...
  XLSX.writeFile(
    workbook,
    `laporan_jurnal_umum_${formatTanggal(tanggalMulai.value)}_s.d_${formatTanggal(tanggalAkhir.value)}.xlsx`,
  )
}
...
function exportToPDF() {
  ...
  // Judul di tengah
  const title = `Laporan Jurnal Umum Tanggal ${formatTanggal(tanggalMulai.value)} s.d ${formatTanggal(tanggalAkhir.value)}`
  ...
  doc.save(
    `laporan jurnal umum ${formatTanggal(tanggalMulai.value)} s.d ${formatTanggal(tanggalAkhir.value)}.pdf`,
  )
}
