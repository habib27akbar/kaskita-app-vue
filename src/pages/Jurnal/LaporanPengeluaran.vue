<template>
  <q-page padding>
    <div class="q-gutter-md">
      <q-card>
        <q-card-section>
          <div class="text-h6">Laporan Jurnal pengeluaran</div>
        </q-card-section>
...
const columns = [
  { name: 'tanggal_kredit', label: 'Tanggal', field: 'tanggal_kredit', align: 'left' },
...
async function filterData() {
  loading.value = true
  try {
    const res = await api.get(`${API_URL}/laporan/pengeluaran`, {
      params: {
...
const exportToExcel = () => {
  ...
  XLSX.writeFile(
    workbook,
    `laporan_pengeluaran_${formatTanggal(tanggalMulai.value)}_s.d_${formatTanggal(tanggalAkhir.value)}.xlsx`,
  )
}
...
function exportToPDF() {
  ...
  // Judul di tengah
  const title = `Laporan Pengeluaran Tanggal ${formatTanggal(tanggalMulai.value)} s.d ${formatTanggal(tanggalAkhir.value)}`
  ...
  autoTable(doc, {
    ...
    head: [
      [
        ...
        { content: 'Debet', styles: { halign: 'center' } },
      ],
    ],
    body: filteredData.value.map((row) => [
      formatTanggal(row.tanggal_kredit),
      row.nama_coa_kredit,
      row.nama_coa_debet,
  ...
  doc.save(
    `laporan pengeluaran ${formatTanggal(tanggalMulai.value)} s.d ${formatTanggal(tanggalAkhir.value)}.pdf`,
  )
}
